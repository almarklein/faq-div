...
function t(o,m){let p=a[o];if(p){p.toggle_one(m);}}
function l(ref_node,o){var u=ref_node.dataset;a[o]={'onhash':v,'toggle_one':w};var x=[[null]];var y={};var z=[];var aa=document.createElement('input');var ab=document.createElement('div');ab.classList.add('search-info');aa.setAttribute('type','text');aa.setAttribute('placeholder',u.searchPlaceholder||'Search FAQ ...');aa.addEventListener('input',ac);aa.className='search';if(u.search=='false'){aa.style.display='none';}
...
am.appendChild(ao);if(u.collapse!='false'){am.classList.add('collapsible');am.classList.add('collapsed');an.setAttribute('onclick','faqdiv.toggle("'+o+'","'+m+'");');let ap=document.createElement('a');ap.className='collapse-icon';an.appendChild(ap);}
if(u.link!='false'){let aq=document.createElement('a');aq.className='link-icon';aq.setAttribute('href','#'+o+':'+m);aq.setAttribute('onclick','event.stopPropagation();');an.appendChild(aq);}
...
function v(m){var at=y[m];if(at){at.node.classList.remove('hidden');at.node.classList.remove('collapsed');at.node.scrollIntoView();r(at.node);}else if(m){aa.value=m.replace(new RegExp('-','g'),' ');ac();}}
function w(m){let at=y[m];at.node.classList.remove('hidden');if(at.node.classList.contains('collapsed')){if(u.collapse=='allbutone'){for(let au in y){let av=y[au];av.node.classList.add('collapsed');}}
at.node.classList.remove('collapsed');}else if(at.node.classList.contains('collapsible')){at.node.classList.add('collapsed');}}
function ac(){var aw='ljbsdfaljhsbdkey';aw;var ax=aa.value.toLowerCase().replace(',','');var ay=[];for(let az of ax.split(' ')){if(az.length>0){ay.push(az);}}
var ba=' how can i , and , or , is ';var bb=[];for(let bc=0;bc<=ay.length;bc++){for(let bd=bc+1;bd<=ay.length;bd++){let be=ay.slice(bc,bd).join(' ');if(ba.indexOf(' '+be+' ')<0){bb.push([bd-bc,be]);}}}
bb.sort(function(a,b){return b[0]-a[0];});if(bb.length==0){as();ab.innerHTML='';for(let m in y){let at=y[m];at.node.classList.remove('hidden');if(at.node.classList.contains('collapsible')){at.node.classList.add('collapsed');}}
for(let j of z){j.classList.remove('hidden');}}else{for(let m in y){let at=y[m];at.node.classList.add('hidden');at.node.classList.remove('collapsed');}
for(let j of z){j.classList.add('hidden');}
var bf=[];for(let m in y){let at=y[m];let bg=0;for(let n=0;n<bb.length;n++){let k=bb[n][0];let bh=bb[n][1];if(at.headertext.indexOf(bh)>=0){bg+=5*k;}
if(at.text.indexOf(bh)>=0){bg+=1*k;}}
bf.push([bg,at.node]);}
bf.sort(function(a,b){return b[0]-a[0];});as();var bi=0;for(let n=0;n<bf.length;n++){let bg=bf[n][0];let j=bf[n][1];j.classList.add('hidden');if(bg>0&&n<16){let bj=j.cloneNode(true);bj.classList.add('searchresult');ref_node.appendChild(bj);bi+=1;bj.classList.remove('hidden');if(bj.classList.remove('collapsible')){if(n<5){bj.classList.remove('collapsed');}else{bj.classList.add('collapsed');}}}}
let bk=u.searchInfo||'COUNT results';ab.innerHTML=bk.replace('COUNT',bi).replace('SUBJECT',aa.value)+'<br>';}}}
